...
# --- 1. 페이지 설정 ---
st.set_page_config(page_title="AI 퀀트 V32 (YF Fix)", layout="wide")
st.title("🤖 AI 퀀트 스크리너 V32 (Final Logic)")
st.markdown("미국 데이터 수집 방식을 **순정 모드**로 변경하여 충돌을 해결했습니다.")
...
def analyze_data(country, index, sector):
    ...
    if country == "미국 (US)":
        ...
        bar = st.progress(0, text=f"🇺🇸 {sector} 데이터 수집 중... (YF Native)")
        for i, t in enumerate(target_tickers):
            try:
                # [핵심 수정] session=session 제거. yfinance가 알아서 처리하게 둠.
                ticker = yf.Ticker(t)
    ...
    # ==========================================
    # 🇰🇷 한국 시장 (네이버 + Custom Session)
    # ==========================================
    else:
        session = get_session() # 한국장은 여전히 세션 필요
        ...
        if all_dfs:
            ...
            for _, row in final_df.iterrows():
                try:
                    ...
                    debt = 0 # 한국장은 상세 조회 시 속도 문제로 0 처리
...
if st.button("🚀 데이터 분석 시작", type="primary"):
    ...
    if not df.empty:
        ...
        st.error("데이터 수집 실패. 서버가 너무 혼잡하거나 일시적 오류입니다.")
